...
const Bookmarks = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      <h1 className="text-4xl font-bold text-center mb-8">Bookmarked Articles</h1>
      {bookmarks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {bookmarks.map((bookmark, index) => (
            <div key={index} className="bg-white shadow-md rounded-lg overflow-hidden">
              {bookmark.urlToImage && (
                <img src={bookmark.urlToImage} alt={bookmark.title} className="w-full h-48 object-cover" />
              )}
              <div className="p-4">
